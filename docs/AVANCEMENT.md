...
0. **📍 ÉTAT AU 3 AOÛT — FRONT F1→F6 TERMINÉ ✅** (accueil · catégorie · filtres sous-catégorie · fiche produit complète : galerie carrousel + configurateur WhatsApp + accordéons + similaires · recherche · pages légales + bandeau cookies · contact · 404 · SEO : meta description, robots.txt, sitemap.xml, JSON-LD). **PLAN :**
   - **DEMAIN MATIN — peaufinage + déploiement** (plan acté avec Serife) :
     1. **Logo + Hero** : ajouter le vrai logo (header/footer) + une image **hero lifestyle** sur l'accueil (cf. A-TRAITER « image du hero » — asset statique `public/images/`, pas la table media).
     2. **Données** : Serife ajoute **~5 produits par catégorie/sous-catégorie** (à voir) → catalogue plus fourni (rend démontrables : filtres sous-cat, similaires, swatches « +N », promos/nouveautés).
     3. **Lisser l'ACCUEIL + les PAGES CATÉGORIES** (même soin design que la fiche produit — homogénéité Montserrat, espacements, cartes).
     4. **WhatsApp flottant → icône FA** (`fa-whatsapp`, CSS `color:#fff`+`font-size` — leftover d'aujourd'hui, rapide).
     5. **Noter les essentiels** au fil de l'eau.
     6. **DÉPLOIEMENT o2switch** — ⚠️ vérifier extensions serveur (**GD+WebP, exif**) ; remplacer `TON-DOMAINE.fr` dans `public/robots.txt` + domaine sitemap ; `.env.local` prod (`APP_ENV=prod`+`DATABASE_URL`) ; `composer install --no-dev` ; **compiler CSS Tailwind en local** + `asset-map:compile` ; migrations ; doc root `public/` ; permissions (`var/`, `public/uploads/`, `public/media/cache/`) ; **ré-uploader les photos** produits ; créer compte admin (`app:create-admin`).
   - **MERCREDI** : démarrer le **DOSSIER** (nouvelle conversation, `docs/DOSSIER.md` déjà en place).
   - **Sessions dédiées à part** : rédiger le **contenu légal** (mentions/confidentialité, posture juriste + recherches) et les **vraies meta descriptions/titles** (passe contenu SEO).

## 🎨 PASSE DESIGN — EN COURS (démarrée 4/8)
- **Décision typo** : on **GARDE Montserrat partout** (Playfair/Inter écartés). → homogénéiser sur toutes les pages (à vérifier).
- **FICHE PRODUIT refondue façon Micadoni ✅** : nom plus gros (`clamp`) · prix actuel > ancien barré · « 2/3/4× SANS FRAIS » sous le prix (Brillance Home le propose vraiment) · configurateur avec **labels de choix** (« Tissu : … / Coloris : … » live) + swatches **« +N »** (révèle les extras au clic, `maxFabrics=4`/`maxColors=6`) · **menu déroulant Configuration** = variantes de la même famille (`family.products` filtrées actives, navigate au `change`) · **buybox épurée** (réassurance + CTA WhatsApp vert pleine largeur, plus de boîte) · **encadré sur-mesure** (champ dimensions → WhatsApp pré-rempli, placé sous Livraison) · accordéons Description/Dimensions/Livraison stylés (+/−, natif `<details>`) · **savoir-faire remplacé par 3 encadrés alignés** (Garantie · Sur-mesure · Paiement) grid 3 col desktop / empilé mobile · « Vous aimerez aussi » conservé.
- **RESTE passe design** : bouton **WhatsApp flottant → icône FA** (`fa-whatsapp`, CSS `color:#fff`+`font-size` — donné, à confirmer fait) · appliquer le même soin aux **autres pages** (accueil, catégorie, recherche, contact, légales, 404) · textes des 3 encadrés + sur-mesure à adapter à la réalité commerciale · A-TRAITER : swatch « +N » se teste avec `maxColors=1`, favoris, limite swatches, variante image tissu.
...
3. **Où on en est** : back-office + médias + LiipImagine ✅ · **FRONT F1 (fondations) ✅** · **FRONT F2 (accueil + finition layout : menu header dynamique, burger mobile, bandeau défilant) ✅** · **FRONT F3 (page catégorie) ✅** · **FRONT F3+ (filtres sous-catégorie) ✅ (30/07)** · **FRONT F4 (fiche produit) ✅ (31/07)** · **F5 (recherche, pages légales, bandeau cookies, contact, 404) ✅** · **F6 (SEO : meta description, robots.txt, sitemap.xml, JSON-LD) ✅**. **PROCHAIN PAS = passe DESIGN CSS (polir tout le site) → DÉPLOIEMENT o2switch → DOSSIER**. Restent aussi : contenus à rédiger (légal + SEO), points A-TRAITER si le temps. (la + grosse : galerie médias, configurateur affiché tissus/couleurs/variantes/modules, devis WhatsApp pré-rempli, accordéons, produits similaires, SEO metaTitle/metaDescription). Puis **F5** (recherche/contact/légales/404), **F6** (SEO sitemap/robots/JSON-LD).
...
**PROGRESS F4 (31/07)** : squelette ✅ · galerie ✅ · **configurateur AFFICHAGE ✅** (couche 1 : `.config` avec tissus `product.fabrics` en boutons + couleurs `product.colors` en pastilles `style="background: hex"`, rendu tolérant `hex starts with '#' ? hex : '#' ~ hex` car données parfois sans `#` → valider le hex sur Color = V2) · **CTA WhatsApp ✅** (`btn-devis`, `wa.me/{{ whatsapp_number }}?text={{ (…)|url_encode }}`, pré-rempli nom produit) · **accordéons ✅** (`<details>` natif 0 JS, description/dimension avec `|nl2br`, livraison statique) · **savoir-faire ✅** (statique) · **similaires ✅** (bloc 5, `findSimilar` leftJoin + orX sous-cat OU famille) · **Phase B mise en page 2 colonnes ✅** (`.product` grid, galerie gauche / infos+accordéons droite en desktop, empilé mobile ; savoir-faire+similaires pleine largeur dessous) · **Phase C interactivité ✅** (sélection tissu/couleur au clic délégué, `.cfg-selection`, devis WhatsApp reconstruit dynamiquement via `data-wa`/`data-product` + `encodeURIComponent`, `updateDevis()` au chargement). **→ F4 COMPLÈTE, commitée le 31/07.** RESTE (reporté) : SEO metaTitle/description fallback (F6) · passe **design CSS** (Serife n'aime pas le style actuel, à polir globalement après F5 — cf. plus bas) · limite swatches (A-TRAITER) · variante photo shownFabric/shownColor (V2). DÉCISIONS : pas de « variante montrée sur photo » (shownFabric/shownColor) → V2 (cf. A-TRAITER) ; configurateur = afficher toutes options, 1ʳᵉ pré-sélectionnée par défaut.
