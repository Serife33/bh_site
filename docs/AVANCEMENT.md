...
0. **📍 ÉTAT AU 3 AOÛT — FRONT F1→F6 TERMINÉ ✅** (accueil · catégorie · filtres sous-catégorie · fiche produit complète : galerie carrousel + configurateur WhatsApp + accordéons + similaires · recherche · pages légales + bandeau cookies · contact · 404 · SEO : meta description, robots.txt, sitemap.xml, JSON-LD). **PLAN :**
   - **DEMAIN (4/8)** : (1) **PASSE DESIGN CSS** — polir TOUT le site (Serife n'aime pas le style actuel → harmoniser couleurs/espacements/typo façon Japandi ; voir points design dans A-TRAITER dont bouton WhatsApp flottant → icône FA `fa-whatsapp`). (2) **DÉPLOIEMENT o2switch** — ⚠️ remplacer `TON-DOMAINE.fr` dans `public/robots.txt` + le domaine du sitemap ; `.env.local` prod (`APP_ENV=prod` + `DATABASE_URL`) ; `composer install --no-dev` ; **`asset-map:compile`** + compiler le CSS Tailwind en local ; migrations ; doc root sur `public/` ; permissions (`var/`, `public/uploads/`, `public/media/cache/`).
   - **MERCREDI (5/8)** : démarrer le **DOSSIER** (nouvelle conversation, `docs/DOSSIER.md` déjà en place).
